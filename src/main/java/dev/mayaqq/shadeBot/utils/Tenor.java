...
//STOLE CODE LOL BUT WHO CARES THIS IS PRIVATE BOT!!!! (also fixed because the code didn't work)
...
public class Tenor {
    ...
    public static String returnGIF(String searchTerm) {
        ...
        for(int i = 0 ; i < jsonArray.length(); i++) {
            list.add(jsonArray.getJSONObject(i).getJSONObject("media").getJSONObject("0").getJSONObject("gif").getString("url"));
            url = jsonArray.getJSONObject(i).getJSONObject("media").getJSONObject("0").getJSONObject("gif").getString("url");
        }
        return url;
    }
